...
function Invoke-TrivyImageScan {
    ...
    docker run --rm `
        ...
        $ImageName 2>&1 | Tee-Object -FilePath $SCAN_LOG -Append | Out-Null
    
    if ($LASTEXITCODE -eq 0) {
        Write-Host "  Image scan completed" -ForegroundColor $GREEN
        "Image scan completed successfully" | Out-File -FilePath $SCAN_LOG -Append
    } else {
        Write-Host "    Image scan completed with warnings" -ForegroundColor $YELLOW
        "Image scan completed with warnings" | Out-File -FilePath $SCAN_LOG -Append
    }
}
...
function Invoke-ContainerImageScan {
    ...
    if ($DockerFiles.Count -gt 0) {
        ...
        "Found Docker files: $($DockerFiles.Name -join ', ')" | Out-File -FilePath $SCAN_LOG -Append
        ...
        foreach ($dockerfile in $DockerFiles) {
            ...
            if ($LASTEXITCODE -eq 0) {
                ...
                "Failed to build image from $($dockerfile.Name)" | Out-File -FilePath $SCAN_LOG -Append
            }
        }
...
function Invoke-BaseImageScan {
    ...
    # Use approved Bitnami hardened images (matches configuration/approved-base-images.conf)
    $BaseImages = @("bitnami/node:latest")
...
function Invoke-FilesystemScan {
    ...
    "Filesystem scan target: $TargetDir" | Out-File -FilePath $SCAN_LOG -Append
    ...
    docker run --rm `
        ...
        --output "/output/$OutputFile" `
        --severity HIGH,CRITICAL `
        "/workspace/$TargetDir" 2>&1 | Tee-Object -FilePath $SCAN_LOG -Append | Out-Null
